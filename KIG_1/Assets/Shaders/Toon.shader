...
Shader "Toon" {
	Properties{
		...
		_OutlineThickness("Outline Thickness", Range(0,1)) = 5.1
...
#pragma fragment frag
	...
	float outlineStrength = saturate((dot(input.normalDir, input.viewDir) - _OutlineThickness) * 3000);


	//float3 ambientLight = (1 - diffuseCutoff) * _UnlitColor.xyz; //adds general ambient illumination
	float3 ambientLight = (diffuseCutoff) * _UnlitColor.xyz; //adds general ambient illumination
